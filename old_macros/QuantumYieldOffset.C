...
/* A mini experiment of what happens when the correction curve is offset by a
 ...
 * of them.
 */
void QuantumYieldOffset(char* solvent_data_file = "",
                     char* in_beam_data = "",
...
void get_L_a(FILE* data, int min, int max) {
    ...
    for (int j = 0; j < num_points; j++) {
        y[j] -= baseline;
        if (is_corr & (j >= 5)) y[j] *= corr[j + offset_ref - offset_data + 5];
...
FILE* open_correction(char* correction) {
    if (cmpfirst(correction, "default")) {
        is_corr = 1;
        inc = 10;
        return fopen("correction/defaultCorrectionData.txt", "r");
    } else if (cmpfirst(correction, "quanta")) {
        is_corr = 2;
        corrheader = 6;
        inc = 2;
        return fopen("correction/emcorr-sphere-quanta.txt", "r");
    } else if (cmpfirst(correction, "cyclohexane")
               || cmpfirst(correction, "cyhx")) {
       is_corr = 3;
       corrheader = 0;
       inc = 10;
       return fopen("correction/CyhxCorrectionData.txt", "r");
    } else if (cmpfirst(correction, "normalizer")) {
       is_corr = 4;
       corrheader = 0;
       inc = 1;
       return fopen("correction/CyhxCorrectionDataB2.txt", "r");
    }
